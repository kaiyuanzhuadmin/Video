...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'kaiyuantv_play_records';
const FAVORITES_KEY = 'kaiyuantv_favorites';
const SEARCH_HISTORY_KEY = 'kaiyuantv_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'kaiyuantv_cache_';
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('kaiyuantv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('kaiyuantv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
